...
async def user_jwt_authentication(request: Request, authorization: str = Header(None, description='token')) -> None:
    """
    jwt token认证
    将方法加入到需要认证的路由上即可实现认证
    :param request:  request
    ...
    if authorization:
        try:
            user = get_token_userdata(authorization)
            result = await User.get_or_none(id=user['user_id'], deleted=False)
            if not result:
                raise HTTPException(403, '账号不存在')
            if result.enabled is False:
                raise HTTPException(200, '账号已被禁用，请联系管理员')
...
def get_token(user_pk: int, _type: str, expiration: int = 60 * 60 * 24 * 5) -> str:
    """
    获取token
    :param user_pk: 用户主键
...
def get_token_userdata(token: str, _type: str = 'user') -> dict:
    """
    token返回data
    :param token: jwt token
    :param _type: 用户类型 admin or user
